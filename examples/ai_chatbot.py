    ...
    try:
        ...
        async with request.app.get_db_session() as session:
            chat_session = ChatSession(
                id=session_id,
                user_id=validated_data.get('user_id'),
                title=validated_data.get('title', f"Chat Session {datetime.utcnow().strftime('%Y-%m-%d %H:%M')}"),
                model_name=validated_data['model_name'],
...
async def get_chat_history(request: Request, session_id: str):
    ...
    try:
        ...
        return JSONResponse(
            {"error": "Failed to get history", "details": str(e)},
            status_code=500
        )
...
async def startup():
    ...
    personalities = [
        ("friendly_chat", "friendly", "You are a friendly and enthusiastic AI assistant who loves to help and chat!"),
        ("professional_chat", "professional", "You are a professional AI assistant focused on providing accurate and helpful information."),
        ("quirky_chat", "quirky", "You are a quirky and fun AI assistant with a unique personality and sense of humor!")
    ]
