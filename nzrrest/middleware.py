...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def _check_rate_limit(self, key: str, now: float) -> bool:
        ...
        return (
            self._check_and_clean_counter(self.minute_counters[key], now, 60, self.calls_per_minute) and
            self._check_and_clean_counter(self.hour_counters[key], now, 3600, self.calls_per_hour) and
            self._check_and_clean_counter(self.day_counters[key], now, 86400, self.calls_per_day)
        )
