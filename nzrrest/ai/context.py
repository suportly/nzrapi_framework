...
class ContextManager:
    ...
    async def create_context(self, context_id: str, metadata: Optional[Dict[str, Any]] = None,
                           ttl: Optional[int] = None) -> ContextData:
        ...
        async with self._lock:
            ...
            if len(self.contexts) >= self.config.max_contexts:
                ...
                if len(self.contexts) >= self.config.max_contexts:
                    oldest_id = min(self.contexts.keys(), 
        ...
        async with self._lock:
            ...
            for context_id, context in list(self.contexts.items()):
                ...
                contexts.append({
                    ...
                    "ttl": context.ttl,
                    "expires_at": (context.updated_at + timedelta(seconds=context.ttl)).isoformat() if context.ttl else None
                })
